...
export class RoleMembersTabComponent {
  ...
  private _onRemoveSelected(): void {
    ...
    this._confirm
      .confirm({
        title: ROLES_KEYS.detail.members.revokeConfirmTitle,
        message: ROLES_KEYS.detail.members.revokeConfirmMessage,
        yes: ROLES_KEYS.detail.members.revokeConfirmYes,
        no: ROLES_KEYS.detail.members.revokeConfirmNo,
      })
      .subscribe((confirmed) => {
        if (!confirmed) return;
        Promise.all(

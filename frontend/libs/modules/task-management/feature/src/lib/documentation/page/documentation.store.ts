...
interface TaskManagementDocumentationBundle {
...
async function loadBundle(locale: 'pl-PL' | 'en-US'): Promise<TaskManagementDocumentationBundle> {
  if (locale === 'en-US') {
...
@Injectable()
export class TaskManagementDocumentationStore {
  ...
  public readonly searchResults = computed<readonly ErpDocumentationSearchResult[]>(() => {
    ...
    return (this._bundleResource.value()?.search ?? [])
      ...
      .slice(0, 8)
      .map((entry) => ({ ...entry, score: 1, route: ['/task-management', 'documentation', entry.slug] }));
  });
}

...
| Task Management — faza 0 (mikroserwis, `Project`/`Issue`, licznik klucza, schemat stanów w seedzie, lista i karta po kluczu) | ✅ | [`task-management.md`](./task-management.md) §13 — zweryfikowane end-to-end: migracja `InitialTaskManagementSchema`, seed (schemat systemowy + 2 projekty + 11 zgłoszeń), lista serwerowa, karta po kluczu czytelnym i przejście stanu przez `job`/`job_item`. Przejście spoza schematu odpada kodem `taskmgmt.transition_*`, nie zmieniając zgłoszenia |
| Task Management — tablice z ręczną kolejnością, pola niestandardowe, hierarchia, zlecenia międzydziałowe | 📐 | [`task-management.md`](./task-management.md) fazy 2–7 + [`task-management-pages.md`](../frontend/task-management-pages.md) — projekt; front modułu to nadal pusty szkielet z zaślepką w menu, klient NSwag niewygenerowany |

...
2. **Klucze pod spodem:** Wszystkie atomy UI (`erp-button`, `erp-input-text`, `erp-datepicker`) oraz molekuły/organizmy (`erp-dynamic-filter`, `erp-modal`, `erp-table`) są **"translation-aware"**. Tłumaczenie odbywa się pod spodem bezpośrednio w ich szablonach HTML za pomocą pipe'a `erpTranslate`, np.:
   ```html
...
> 
> Zapis `providers: [provideSharedTranslations()]` w dekoratorze `@Component` tworzy lokalny wstrzykiwacz potomny (child injector), co nadpisuje i blokuje główny wstrzykiwacz scope'u Transloco (np. `product`). Powoduje to błędy typu `Missing translation for 'product.base.tabs.products'`.
>
> **Zamiast tego:**
> - Używaj wyłącznie pipe'a `erpTranslate` w HTML.
...
## 3. Automatyczne Ładowanie Translation Providers w Modalach

> [!TIP]
> **Definicje modali (np. `SetPriceModalDefinition`) NIE POWINNY wywoływać metody `.setProviders(provideProductTranslations())` w builderze.**
>
> Zamiast tego `ErpModalService` automatycznie pobiera i wstrzykuje odpowiednie providery przy ładowaniu modali z remota. W tym celu każdy kontrakt remota powinien eksponować funkcję `getModalProviders()` w pliku `entry.modals.ts`:
>
> ```typescript
...
## 4. Automatyczne Generowanie `keys.ts` (Generator)
...
## 5. Struktura Folderu Translation

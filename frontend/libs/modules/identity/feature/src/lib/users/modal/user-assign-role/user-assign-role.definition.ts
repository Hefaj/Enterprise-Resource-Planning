...
import { UserAssignRoleStepComponent } from './user-assign-role.step';
...
export type UserAssignRoleMetadata = ErpBatchMetadata;
...
export class UserAssignRoleModalDefinition
  implements ErpModalDefinition<BatchCommandOfUserAssignRoleCommandAndSearchUserAccountRequest, UserAssignRoleMetadata>
{
  public readonly id = USER_ASSIGN_ROLE_MODAL_ID;
  ...
  public build(
    ...
    metadata?: UserAssignRoleMetadata,
  ): ErpModalConfig<BatchCommandOfUserAssignRoleCommandAndSearchUserAccountRequest, UserAssignRoleMetadata> {
    ...
    return ErpModalBuilder.modal<BatchCommandOfUserAssignRoleCommandAndSearchUserAccountRequest, UserAssignRoleMetadata>((b) =>
      b
        .setTitle([USERS_KEYS.title, USERS_KEYS.commands.assignRole.modalTitle])
        .setCommand(command)
        .setMetadata(metadata)
        .addStep(USERS_KEYS.commands.assignRole.label, UserAssignRoleStepComponent)
        .setSaveLabel(USERS_KEYS.commands.assignRole.submitButton)
        .setOnSave(async (cmd) => {
          await this._userOrchestrator.assignRoleMultipleAsync(cmd, USER_ASSIGN_ROLE_MODAL_ID);
        }),
    );
  }
}

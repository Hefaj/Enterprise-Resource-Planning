...
| Task Management — fazy 0–2 (mikroserwis, `Project`/`Issue`, komentarze i historia, **tablica z ręczną kolejnością kart**) | ✅ | [`task-management.md`](./task-management.md) §13 — faza 0 zweryfikowana end-to-end; faza 1: komentarze, `issue_activity`, załączniki; faza 2: `Board`/`board_column`/`board_card`, rank jako łańcuch (`BoardRank`), `BoardSetCardPosition` liczący rank z sąsiadów w transakcji, rebalans `[ClusterSafe]` pod dzierżawą `taskmgmt:board-rank-rebalance`, kanał `taskmgmt.board` |
| Task Management — pola niestandardowe, hierarchia, zlecenia międzydziałowe, sprinty | 📐 | [`task-management.md`](./task-management.md) fazy 3–7 + [`task-management-pages.md`](../frontend/task-management-pages.md) — projekt; front ma dziś listę zgłoszeń, kartę zgłoszenia i tablicę, klient NSwag wygenerowany |

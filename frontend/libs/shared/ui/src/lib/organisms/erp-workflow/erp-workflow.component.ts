import { ChangeDetectionStrategy, Component, computed, ElementRef, HostListener, inject, model, input, signal, effect, contentChild, TemplateRef, AfterViewInit, ViewChild } from '@angular/core';
...
export class ErpWorkflowComponent implements AfterViewInit {
  ...
  deleteNode(nodeId: string): void {
    ...
    this.edges.update(es => es.filter(e => e.sourceId !== nodeId && e.targetId !== nodeId));
  }
  ...
  onNodeHeightChange(event: { id: string, height: number }): void {
    this.nodes.update(ns => ns.map(n => n.id === event.id ? { ...n, height: event.height } : n));
  }

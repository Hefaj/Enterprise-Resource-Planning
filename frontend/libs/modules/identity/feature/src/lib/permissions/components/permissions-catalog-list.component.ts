  ...
  template: `
    <div class="flex flex-col h-full w-full min-h-0 gap-3 p-4">
      <div class="flex flex-col gap-1">
        <h1 class="page-title">{{ IDENTITY_KEYS.permissions.title | erpTranslate }}</h1>
        <p class="page-subtitle">{{ IDENTITY_KEYS.permissions.subtitle | erpTranslate }}</p>
      </div>
      ...
      <div class="flex-1 min-h-0 overflow-y-auto flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-1">
              @for (entry of group.entries; track entry.code) {
                ...
                >
                  ...
                  @if (entry.isObsolete) {
                    <span class="badge">{{ IDENTITY_KEYS.permissions.obsoleteBadge | erpTranslate }}</span>
                  }
                </button>
              }
            </div>
        ...
        @if (groups().length === 0) {
          <p class="empty">{{ IDENTITY_KEYS.permissions.emptyMessage | erpTranslate }}</p>
        }
      </div>
    </div>
  `,
  styles: [
    `
      ...
      .entry-row {
        ...
        padding: 0.4rem 0.6rem; border-radius: 0.375rem; border: 1px solid transparent;
        background: none; cursor: pointer; width: 100%;
      }
      .entry-row:hover { background: var(--tui-background-neutral-1); }
      .entry-row.selected { background: var(--tui-background-neutral-1); border-color: var(--tui-border-normal); }
      .entry-code { font-family: monospace; font-size: 0.8rem; min-width: 14rem; }
      .entry-desc { color: var(--tui-text-secondary); font-size: 0.8rem; flex: 1; }
      .badge {
        font-size: 0.7rem; padding: 0.1rem 0.4rem; border-radius: 0.25rem;
        background: var(--tui-status-warning-pale); color: var(--tui-status-warning);
      }
      .empty { color: var(--tui-text-secondary); }
    `,
  ],
...
export class PermissionsCatalogListComponent {
  ...
  protected readonly groups = computed<ModuleGroup[]>(() => {
    ...
    return [...byModule.entries()]
      .map(([module, entries]) => ({ module, entries: entries.sort((a, b) => a.code.localeCompare(b.code)) }))

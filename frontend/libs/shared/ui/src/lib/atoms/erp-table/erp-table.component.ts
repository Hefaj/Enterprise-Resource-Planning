...
@Component({
  ...
  template: `
    <div
      class="erp-table-container"
      [class.erp-table--compact]="_compact()"
    >
                    ...
                    >
                      @if (header.subHeaders && header.subHeaders.length > 0) {
                        ...
                        <div
                          ...
                          [class.items-end]="$any(header.column.columnDef.meta)?.['align'] === 'right'"
                          [class.items-center]="$any(header.column.columnDef.meta)?.['align'] === 'center'"
                        >
                          ...
                          @if ($any(header.column.columnDef.meta)?.['subHeader']; as subHeader) {
                            <span
                              class="text-[0.6875rem] leading-none mt-0.5 text-(--tui-text-tertiary) truncate block font-normal w-full"
                              [class.text-right]="$any(header.column.columnDef.meta)?.['align'] === 'right'"
                              [class.text-center]="$any(header.column.columnDef.meta)?.['align'] === 'center'"
                            >
            ...
            @if (_flatDisplayRows().length === 0 && !loading()) {
              <tr>
                <td
                  ...
                  class="p-8 text-center text-(--erp-table-text-secondary)"
                >
              ...
              >
                ...
                >
                  <div class="erp-table__group-content">
                    ...
                    @if (flatRow.loading) {
                      <tui-icon
                        icon="@tui.loader-circle"
                        class="erp-table__group-spinner"
                      />
                    ...
                    @if (_groupActions().length > 0) {
                      <div
                        class="erp-table__group-actions"
                        (click)="$event.stopPropagation()"
                      >
                ...
                @for (cell of _getOrderedCells(row); track cell.id) {
                  ...
                  >
                    <ng-container *flexRender="cell.column.columnDef.cell; props: cell.getContext(); let cellValue">
                      <span
                        [tuiSkeleton]="loading()"
                        class="rounded-sm inline-flex items-center min-w-[3rem] min-h-[1.25rem] max-w-full"
                      >
                        {{ cellValue }}
                      </span>
                    </ng-container>
            ...
            @if (_enableVirtualScroll()) {
              ...
              @if (virtualizer().getVirtualItems().length > 0) {
                <tr>
                  <td
                    [colSpan]="table.getVisibleFlatColumns().length"
                    [style.height.px]="virtualizer().getVirtualItems()[0].start"
                  ></td>
                </tr>
              }
              ...
              @for (virtualRow of virtualizer().getVirtualItems(); track virtualRow.key) {
                ...
                @if (flatRow.kind === 'group') {
                  <ng-container *ngTemplateOutlet="groupRowTpl; context: { $implicit: flatRow, virtualizer: virtualizer(), index: virtualRow.index }"></ng-container>
                } @else {
                  <ng-container *ngTemplateOutlet="leafRowTpl; context: { $implicit: flatRow.row, virtualizer: virtualizer(), index: virtualRow.index }"></ng-container>
                }
              }
              ...
              @if (virtualizer().getVirtualItems().length > 0) {
                <tr>
                  <td
                    [colSpan]="table.getVisibleFlatColumns().length"
                    [style.height.px]="virtualizer().getTotalSize() - virtualizer().getVirtualItems()[virtualizer().getVirtualItems().length - 1].end"
                  ></td>
                </tr>
              }
              ...
              @for (flatRow of _flatDisplayRows(); track _flatRowKey(flatRow); let i = $index) {
                @if (flatRow.kind === 'group') {
                  <ng-container *ngTemplateOutlet="groupRowTpl; context: { $implicit: flatRow, virtualizer: null, index: i }"></ng-container>
                } @else {
                  <ng-container *ngTemplateOutlet="leafRowTpl; context: { $implicit: flatRow.row, virtualizer: null, index: i }"></ng-container>
                }
              }
            } @else {
              <!-- Zwykła pętla -->
              @for (row of table.getRowModel().rows; track row.id; let i = $index) {
                <ng-container *ngTemplateOutlet="leafRowTpl; context: { $implicit: row, virtualizer: null, index: i }"></ng-container>
              }
            }
          </tbody>
                  ...
                  @for (footer of _getOrderedHeaders(footerGroup); track footer.id) {
                    ...
                    >
                      <div
                        ...
                        [class.justify-end]="$any(footer.column.columnDef.meta)?.['align'] === 'right'"
                        [class.justify-center]="$any(footer.column.columnDef.meta)?.['align'] === 'center'"
                      >
                        <span class="truncate block">
            ...
            <div class="p-4 max-w-sm bg-(--tui-background-elevated-1) border border-(--tui-border-normal) rounded-md shadow-lg">
              ...
              <ul class="text-sm space-y-2 list-disc pl-4 text-(--tui-text-secondary)">
                @if (config().enableMultiSort ?? true) {
                  <li>
                    <strong>{{ 'shared.table.help.multiSortTitle' | erpTranslate }}:</strong> {{ 'shared.table.help.multiSortDesc' | erpTranslate }}
                  </li>
                }
                @if (config().enableColumnResizing ?? true) {
                  <li>
                    <strong>{{ 'shared.table.help.resizingTitle' | erpTranslate }}:</strong> {{ 'shared.table.help.resizingDesc' | erpTranslate }}
                  </li>
                }
                @if (config().enableColumnVisibility ?? true) {
                  <li>
                    <strong>{{ 'shared.table.help.visibilityTitle' | erpTranslate }}:</strong> {{ 'shared.table.help.visibilityDesc' | erpTranslate }}
                  </li>
                }
                @if (config().selectionMode === 'multi') {
                  <li>
                    <strong>{{ 'shared.table.help.multiSelectTitle' | erpTranslate }}:</strong> {{ 'shared.table.help.multiSelectDesc' | erpTranslate }}
                  </li>
                }
                @if (config().selectionMode === 'multi' && _isServerMode()) {
                  <li>
                    <strong>{{ 'shared.table.help.serverSelectionTitle' | erpTranslate }}:</strong> {{ 'shared.table.help.serverSelectionDesc' | erpTranslate }}
                  </li>
                }
              </ul>
            </div>
...
export class ErpTableComponent<T> implements AfterViewInit {
  ...
  protected _flatDisplayRows = computed<FlatDisplayRow<T>[]>(() => {
    ...
    if (!cfg) {
      return this.table()
        .getRowModel()
        .rows.map((row) => ({ kind: 'leaf' as const, row }));
    }
  ...
  protected _getOrderedColumns() {
    return [...this.table.getLeftVisibleLeafColumns(), ...this.table.getCenterVisibleLeafColumns(), ...this.table.getRightVisibleLeafColumns()];
  }
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const state: Partial<ErpTableState> | undefined = config.initialState ?? (key ? untracked(() => this.preferencesService.getState(ErpPreferencesType.Table, key)) : undefined);
      ...
      untracked(() => {
        ...
        const finalOrder = currentOrder.length > 0 ? [...currentOrder.filter((id) => newOrder.includes(id)), ...newOrder.filter((id) => !currentOrder.includes(id))] : newOrder;
    ...
    effect(() => {
      ...
      untracked(() => {
        const visibleRowsByGroup = cfg.onVisibleRowsChange ? new Map<string, { group: any; rows: T[] }>() : null;
  ...
  private _applyPendingSizingRescale(viewportWidth: number): void {
    ...
    const rescaled = erpRescaleColumnWidths(this._sizingColumns(), this._columnSizing(), savedViewport, viewportWidth);
  ...
  private _mapErpColumnToTanstack(col: ErpColumnDef<any>): ColumnDef<T> {
    return {
      ...
      cell: col.cell
        ...
        : col.cellRichContent
          ? ({ getValue, row }) =>
              flexRenderComponent(ErpChipCellComponent, {
      ...
      meta: {
        ...
        cellClass: col.cellRichContent ? (col.cellClass ? col.cellClass + ' erp-table__cell--rich-content' : 'erp-table__cell--rich-content') : col.cellClass,
      },
    };
  }
  ...
  // Main Table Instance
  protected table: Table<T> & Signal<Table<T>> = createAngularTable<T>(() => ({
    data: this.items(),
  ...
  protected _columnMenuInfo = computed(() => {
    ...
    const mapColumn = (colOrGroup: ErpColumnDef<any> | ErpColumnGroupDef<any>): any => {
      ...
      if (isColumnGroupDef(colOrGroup)) {
        const children = colOrGroup.columns.map((c) => mapColumn(c)).filter((c) => c !== null);
  ...
  protected onRowClickEvent(rowOriginal: T, event: MouseEvent) {
    ...
    if (this._rowSelectionOnClick() && this.config().selectionMode !== 'none') {
      ...
      const tanstackRow = this.table()
        .getRowModel()
        .rows.find((r) => r.original === rowOriginal);

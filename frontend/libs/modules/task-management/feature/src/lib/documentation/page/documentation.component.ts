...
import { ErpDocumentationLayoutBuilder, ErpDocumentationLayoutComponent, ErpDocumentationSearchBuilder, SHARED_KEYS } from '@erp/shared/ui';
import { ErpDocumentationSearchResult } from '@erp/shared/util';
import { TaskManagementDocumentationStore } from './documentation.store';

@Component({
  selector: 'erp-task-management-documentation',
  standalone: true,
  imports: [ErpDocumentationLayoutComponent],
  providers: [TaskManagementDocumentationStore],
  template: `<erp-documentation-layout [config]="layoutConfig" />`,
  host: { style: 'display: block; height: 100%; min-height: 0;' },
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TaskManagementDocumentationComponent {
  private readonly _store = inject(TaskManagementDocumentationStore);
  ...
  public constructor() {
    effect(() => { const article = this.article(); if (article) this._title.setTitle(article.title); });
    effect(() => { const fragment = this._fragment(); if (fragment) queueMicrotask(() => this._focusHeading(fragment)); });
  }
  ...
  private _selectArticle(articleId: string): void {
    const article = this._store.documentation()?.articles[articleId];
    if (article) void this._router.navigate(['/task-management', 'documentation', article.slug]);
  }

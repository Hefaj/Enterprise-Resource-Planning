...
@Component({
  ...
  template: `
    @if (role(); as r) {
      <div class="flex flex-col h-full w-full gap-2 p-2 overflow-y-auto" erpActionToolbarZone [erpActionToolbarContext]="actionToolbar">
        <erp-action-toolbar [config]="actionToolbar" />

        @if (r.permissions.length === 0) {
          <p class="empty">{{ IDENTITY_KEYS.roles.detail.permissions.emptyMessage | erpTranslate }}</p>
        }
          ...
          @for (code of r.permissions; track code) {
            <span class="chip">
              ...
              @if (canManage() && !r.isSystem) {
                <button type="button" class="chip-remove" (click)="onRemove(code)">×</button>
              }
            </span>
          }
        </div>
  ...
  styles: [
    `
      ...
      .chip-remove {
        ...
        color: var(--tui-text-tertiary); font-size: 0.9rem;
      }
      .chip-remove:hover { color: var(--tui-status-negative); }
    `,
  ],
...
export class RolePermissionsTabComponent {
  ...
  protected readonly actionToolbar = ErpActionToolbarBuilder.create((b) =>
    b
      ...
      .addDefaultGroup((g) =>
        g
          .setId('permissions')
          .setLabel(IDENTITY_KEYS.roles.detail.tabs.permissions)
          .setIcon('@tui.key')
          .addAction((a) =>
            a
              .setId('add-permission')
              .setLabel(IDENTITY_KEYS.roles.commands.addPermission.label)
              .setIcon('@tui.plus')
  ...
  protected onRemove(code: string): void {
    ...
    this._confirm
      .confirm({
        title: IDENTITY_KEYS.roles.detail.permissions.removeConfirmTitle,
        message: IDENTITY_KEYS.roles.detail.permissions.removeConfirmMessage,
        yes: IDENTITY_KEYS.roles.detail.permissions.removeConfirmYes,
        no: IDENTITY_KEYS.roles.detail.permissions.removeConfirmNo,
      })
      .subscribe((confirmed) => {
        if (!confirmed) return;
        this._orchestrator
          .removePermissionAsync({ roleUuid, permissionCode: code })
          .catch((err) => console.error('[RolePermissionsTabComponent] Nie udało się usunąć uprawnienia.', err));
      });
  }
}

...
public sealed class UserAssignRoleMultipleCommandEndpoint
    : BatchEndpointBase<UserAssignRoleCommand, SearchUserAccountRequest>
{
    private readonly IUserAccountQueries _queries;
    private readonly UserBatchValidator _validator;

    public UserAssignRoleMultipleCommandEndpoint(IUserAccountQueries queries, UserBatchValidator validator)
    {
        _queries = queries;
    ...
    protected override Task<ValidationTracker> ValidateTargetsAsync(
        ...
        CancellationToken ct)
        => _validator.ValidateAssignRoleAsync(targets, ct);
}

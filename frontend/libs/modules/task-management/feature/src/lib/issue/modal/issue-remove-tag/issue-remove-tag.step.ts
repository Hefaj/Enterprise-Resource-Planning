...
/** Krok modalu seryjnego odpięcia tagu — patrz {@link IssueAddTagStepComponent}, odwrotna
 * komenda. */
@Component({
  selector: 'erp-task-management-issue-remove-tag-step',
  standalone: true,
  imports: [ErpStepContentComponent],
  template: `<erp-step-content [contentConfig]="formContent" />`,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class IssueRemoveTagStepComponent extends ErpBatchStepBase<
  BatchCommandOfIssueRemoveTagCommandAndSearchIssueRequest,
...
> {
      ...
      b
        ...
        .addBatchTargetsSummary((s) =>
          s
            ...
            .setMessages({
              messageKey: ISSUE_KEYS.commands.removeTag.editMessage,
        ...
        .addFormField(
          ...
          (f) =>
            f
              .setLabel(ISSUE_KEYS.commands.removeTag.tagLabel)
              .setItems(tagOptions)
              .setLabelKey('label')
              .setValueKey('uuid')
              .setStrategy('single'),
  ...
  private async _loadTagsAsync(projectUuid: string | null): Promise<void> {
    ...
    try {
      ...
      console.error('[IssueRemoveTagStepComponent] Nie udało się pobrać tagów projektu.', error);
    }
  }
}

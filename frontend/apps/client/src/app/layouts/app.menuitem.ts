...
export class AppMenuitem {
  ...
  public item = input<any>(null);

  public root = input<boolean>(false);

  public parentPath = input<string | null>(null);

  public isVisible = computed(() => this.item()?.visible !== false);

  public hasChildren = computed(() => this.item()?.items && this.item()?.items.length > 0);

  public hasRouterLink = computed(() => !!this.item()?.routerLink);

  public fullPath = computed(() => {

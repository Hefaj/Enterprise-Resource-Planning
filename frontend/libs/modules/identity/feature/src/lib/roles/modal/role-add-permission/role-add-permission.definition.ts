...
import { RoleAddPermissionStepComponent } from './role-add-permission.step';
...
export interface RoleAddPermissionMetadata extends ErpBatchMetadata {
...
export class RoleAddPermissionModalDefinition
  implements ErpModalDefinition<BatchCommandOfRoleAddPermissionCommandAndSearchRoleRequest, RoleAddPermissionMetadata>
{
  public readonly id = ROLE_ADD_PERMISSION_MODAL_ID;
  ...
  public build(
    ...
    metadata?: RoleAddPermissionMetadata,
  ): ErpModalConfig<BatchCommandOfRoleAddPermissionCommandAndSearchRoleRequest, RoleAddPermissionMetadata> {
    ...
    return ErpModalBuilder.modal<BatchCommandOfRoleAddPermissionCommandAndSearchRoleRequest, RoleAddPermissionMetadata>((b) =>
      b
        .setTitle([ROLES_KEYS.title, ROLES_KEYS.commands.addPermission.modalTitle])
        .setCommand(command)
        .setMetadata(metadata ?? { excludeCodes: [] })
        .addStep(ROLES_KEYS.commands.addPermission.label, RoleAddPermissionStepComponent)
        .setSaveLabel(ROLES_KEYS.commands.addPermission.submitButton)
        .setOnSave(async (cmd) => {
          await this._roleOrchestrator.addPermissionMultipleAsync(cmd, ROLE_ADD_PERMISSION_MODAL_ID);
        }),
    );
  }
}
